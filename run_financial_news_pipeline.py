...
from comment_position import (
    ...
    COMMENT_POSITION_LABELS as SHARED_COMMENT_POSITION_LABELS,
...
_COMMENT_POSITION_LABELS = SHARED_COMMENT_POSITION_LABELS


def _is_comment_position_doc(doc: Optional[Dict[str, Any]]) -> bool:
    return shared_is_comment_position_document(doc)
...
def _run_enrichment_agent(
    ...
    instruction = (
        ...
        "supervisory emphasis, affected registrants/intermediaries, timing signals, and exact statements that merit source review. "
        "Do not spend space on basic metadata such as the document title, speaker, date, source, or publication format unless that "
        "detail is essential to understanding the substance. Do not characterize a speech, testimony, education item, or transcript "
        "as enforcement-related unless the text explicitly mentions investigation, charges, litigation, settlement, penalty, fraud, "
        "suspension, or similar enforcement posture. "
        "Use concise regulator-specific tags and keywords. entities must be list of {name,type,mentions}. "
