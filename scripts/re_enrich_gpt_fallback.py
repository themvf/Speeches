...
def deepseek_feed_analysis(row: Dict[str, Any], topics: List[str], model: str, api_key: str) -> Dict[str, Any]:
    ...
    instructions = (
        ...
        "2-5 concrete red flags, uncertainties, or missing facts. Do not use vague labels like 'potential regulatory risk' "
        "unless tied to a named fact from the input. follow_up_questions: 2-5 item-specific questions about parties, "
        "securities/instruments, dates, rules, procedural posture, jurisdiction, losses, obligations, or market impact "
        "where relevant. keywords, individuals, and entities must be extracted only from the title, source, author, "
        "topics, URL, and excerpt; avoid author fragments and generic media words. Use empty arrays only when the "
        "supplied text truly does not identify a category. "
