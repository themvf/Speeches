...
export async function GET(request:Request){
 ...
 if(!process.env.DATABASE_URL)return ok({status:'not_configured',posts:[],days:[],total:0,limit:5000});
 const sql=neon(process.env.DATABASE_URL);
 try{
  const exists=await sql`SELECT to_regclass('public.crypto_social_posts') AS relation`;
  if(!exists[0]?.relation)return ok({status:'not_started',posts:[],days:[],total:0,limit:5000});
  const [posts,days,total]=await Promise.all([
   sql`WITH matched AS (SELECT DISTINCT p.* FROM crypto_social_posts p JOIN crypto_social_matches m ON m.post_id=p.id
    JOIN crypto_social_windows w ON w.id=m.window_id WHERE w.coin=${coin} AND p.posted_at>='2026-07-25'::timestamptz)
    ...
    FROM matched p JOIN crypto_social_accounts a ON a.id=p.author_id ORDER BY p.posted_at,p.id LIMIT 5000`,
   sql`SELECT (start_at AT TIME ZONE 'UTC')::date::text AS day,count(*)::int AS windows,
    ...
    FROM crypto_social_windows WHERE coin=${coin} AND start_at>='2026-07-25'::timestamptz AND query<>'timeline text match' GROUP BY 1 ORDER BY 1`,
   sql`SELECT count(DISTINCT p.id)::int AS total FROM crypto_social_posts p JOIN crypto_social_matches m ON m.post_id=p.id
    JOIN crypto_social_windows w ON w.id=m.window_id WHERE w.coin=${coin} AND p.posted_at>='2026-07-25'::timestamptz`,
  ]);
  return ok({status:'ready',posts,days,total:total[0].total,limit:5000,start:'2026-07-25',end:new Date().toISOString().slice(0,10)});
