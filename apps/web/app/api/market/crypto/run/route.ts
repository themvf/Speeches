...
export async function GET(request:Request){
 ...
 try{
  ...
  if(!exists[0]?.relation)return ok({status:'not_started',posts:[],days:[],total:0,limit:25000});
  const [posts,days,total]=await Promise.all([
   sql`WITH matched AS (SELECT DISTINCT p.* FROM crypto_social_posts p JOIN crypto_social_matches m ON m.post_id=p.id
    ...
    SELECT p.*,a.handle,a.followers::float AS followers,a.observed_at AS followers_observed_at,
     metrics.likes::float AS likes,metrics.quotes::float AS quotes,metrics.reposts::float AS reposts,metrics.observed_at AS metrics_observed_at,coalesce((SELECT jsonb_agg(jsonb_build_object('target_id',e.target_id,'target',coalesce(b.handle,e.target_id),'kind',e.kind) ORDER BY e.target_id,e.kind)
     FROM crypto_social_edges e LEFT JOIN crypto_social_accounts b ON b.id=e.target_id WHERE e.post_id=p.id),'[]'::jsonb) AS edges
    FROM matched p JOIN crypto_social_accounts a ON a.id=p.author_id
    LEFT JOIN LATERAL (SELECT likes,quotes,reposts,observed_at FROM crypto_social_snapshots WHERE post_id=p.id ORDER BY observed_at DESC,request_id DESC LIMIT 1) metrics ON true
    ORDER BY p.posted_at,p.id LIMIT 25000`,
