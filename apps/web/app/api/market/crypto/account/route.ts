...
export async function GET(request:Request){
 ...
 try{const sql=neon(process.env.DATABASE_URL);
  const [account,profile,activity,posts,summary]=await Promise.all([
  ...
  return withCdnCache(ok({status:'ready',id,account:account[0],latest_profile:profile[0]??null,profile_history:profile,activity,posts,summary}));
