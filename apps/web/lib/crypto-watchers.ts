...
const action=/\b(?:bought|sold|buy|sell|purchase|transfer|deposit|withdraw|accumulat|inflow|outflow|wallet|holder)\w*\b|买入|卖出|钱包|转账|增持|持仓/i;
function template(text:string){return text.toLowerCase().replace(/https?:\/\/\S+|0x[a-f0-9]+|\$[a-z]+|@[\w]+/g,' ').replace(/[\d,.]+/g,'#').replace(/\s+/g,' ').trim();}
export function watcherSignals(text:string){const w=whale.test(text),v=volume.test(text),numeric=amount.test(text);return {whale:w,volume:v,supported:(w&&action.test(text)&&numeric)||(v&&numeric),linked:link.test(text)};}
export function rankWatchers(raw:WatcherPost[],coin='ALL'){
 ...
 const rows=[...groups].flatMap(([id,all])=>{
  ...
  const score=8*Math.min(matched.length,10)+12*Math.min(days,5)+6*Math.min(formats,4)+4*Math.min(linked,5)+(bioMatch?8:0);
  ...
  return [{id,handle:latest.handle,followers:latest.followers??null,followersObserved:latest.followers_observed_at??null,bio,score,
   category:whales&&volumes?'Whale & volume watcher':whales?'Whale watcher':'Volume reporter',
