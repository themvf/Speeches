...
const p:WatcherPost={id:'1',author_id:'a',handle:'alice',text:'$ZCAT whale wallet bought $500K',url:'https://x.com/i/status/1',posted_at:'2026-09-14T12:00:00Z',kind:'original',edges:[],coins:['ZCAT'],followers:100,bio:'Whale tracking alerts'};
...
test('incidental whale commentary is not a transaction report',()=>{assert.equal(watcherSignals('Whales are buying and liking posts. Zcash is $1000.').supported,false);assert.equal(watcherSignals('Smart Wallets: 0 | Market cap $73K').supported,false);});
