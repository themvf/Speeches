...
test('each rule fires only on its own condition and names it',()=>{
 ...
 const s=evaluateSignals({coins:[coin({}),coin({symbol:'PONS',price_now:1.01,price_24h_ago:1,volume_24h:100,volume_prev_24h:100,posts_24h:25})],posts:[post({contract:true}),post({id:'2',text:'whale bought $50K of $ZCAT'}),post({id:'3',author_id:'z',contract:true}),post({id:'4',kind:'repost',contract:true})],watched,firstContract:[{coin:'KNOTS',post_id:'9',handle:'scan',account_id:'s',posted_at:'2026-09-17T05:00:00Z',url:'u'},{coin:'ZCAT',post_id:'8',handle:'old',account_id:'o',posted_at:'2026-08-01T00:00:00Z',url:'u'}],windowStart:start,topAuthorShare:[{coin:'PONS',share:.7}]});
