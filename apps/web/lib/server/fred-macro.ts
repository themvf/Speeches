...
/**
 * Cache lifetime per publication cadence.
 *
 * Worst-case staleness for a new print equals the lifetime, so anything slower
 * than daily gets an hour: a CPI release at 8:30 ET appears by 9:30 at the
 * latest, which is the right trade for a research dashboard that also carries a
 * release calendar. Daily market rates keep the original fifteen minutes.
 */
const CADENCE_REFRESH_SECONDS: Readonly<Record<FredCadence, number>> = {
  daily: FRED_MACRO_CACHE_SECONDS,
  weekly: 60 * 60,
  monthly: 60 * 60,
  quarterly: 60 * 60,
};
...
/**
 * Series metadata - the publication frequency and FRED's own ingest time - is
 * a second request per indicator, and it was on the same fifteen-minute cycle
 * as the numbers. Frequency never changes and an ingest time changes at most a
 * few times a day, so once a day is ample and this halves the route's traffic.
 */
const METADATA_REFRESH_SECONDS = 24 * 60 * 60;

/** Cache lifetime for a series publishing at this cadence. */
export function seriesRefreshSeconds(cadence: FredCadence): number {
  return CADENCE_REFRESH_SECONDS[cadence];
}

/** Shown on the card when FRED's own metadata is unavailable. */
const FALLBACK_FREQUENCY_LABEL: Readonly<Record<FredCadence, string>> = {
  daily: "Daily",
  weekly: "Weekly",
  monthly: "Monthly",
  quarterly: "Quarterly",
...
export interface FredMacroDefinition {
  ...
  limit: number;
  /**
   * How often the source actually publishes. Drives how long the observation
   * response is cached: asking a monthly series 96 times a day for a number
   * that changes twelve times a year is the bulk of this route's FRED traffic.
   */
  cadence: FredCadence;
  scale?: number;
}
...
export const FRED_MACRO_DEFINITIONS: readonly FredMacroDefinition[] = [
  {
    id: "real_gdp_growth", seriesId: "A191RL1Q225SBEA", cadence: "quarterly", releaseId: 53, label: "Real GDP Growth",
  ...
  {
    id: "cpi_inflation", seriesId: "CPIAUCSL", cadence: "monthly", releaseId: 10, label: "CPI Inflation",
  ...
  {
    id: "nonfarm_payrolls", seriesId: "PAYEMS", cadence: "monthly", releaseId: 50, label: "Nonfarm Payrolls",
  ...
  {
    id: "unemployment_rate", seriesId: "UNRATE", cadence: "monthly", releaseId: 50, label: "Unemployment Rate",
  ...
  {
    id: "effective_fed_funds", seriesId: "DFF", cadence: "daily", releaseId: 18, label: "Effective Fed Funds Rate",
  ...
  {
    id: "yield_curve_10y2y", seriesId: "T10Y2Y", cadence: "daily", releaseId: 304, label: "10Y-2Y Treasury Spread",
  ...
  {
    id: "retail_sales_growth", seriesId: "RSAFS", cadence: "monthly", releaseId: 9, label: "Retail Sales",
  ...
  {
    id: "industrial_production_growth", seriesId: "INDPRO", cadence: "monthly", releaseId: 13, label: "Industrial Production",
  ...
  {
    id: "core_pce_inflation", seriesId: "PCEPILFE", cadence: "monthly", releaseId: 54, label: "Core PCE Inflation",
  ...
  {
    id: "breakeven_inflation_10y", seriesId: "T10YIE", cadence: "daily", releaseId: 304, label: "10Y Breakeven Inflation",
  ...
  {
    id: "producer_price_inflation", seriesId: "PPIFIS", cadence: "monthly", releaseId: 46, label: "Producer Price Inflation",
  ...
  {
    id: "initial_claims", seriesId: "ICSA", cadence: "weekly", releaseId: 180, label: "Initial Jobless Claims",
  ...
  {
    id: "average_hourly_earnings_growth", seriesId: "CES0500000003", cadence: "monthly", releaseId: 50, label: "Average Hourly Earnings",
  ...
  {
    id: "labor_force_participation", seriesId: "CIVPART", cadence: "monthly", releaseId: 50, label: "Labor Force Participation",
  ...
  {
    id: "job_openings", seriesId: "JTSJOL", cadence: "monthly", releaseId: 192, label: "Job Openings",
  ...
  {
    id: "sahm_rule", seriesId: "SAHMREALTIME", cadence: "monthly", releaseId: 456, label: "Sahm Rule Indicator",
  ...
  {
    id: "national_financial_conditions", seriesId: "NFCI", cadence: "weekly", releaseId: 221, label: "National Financial Conditions",
  ...
  {
    id: "financial_stress", seriesId: "STLFSI4", cadence: "weekly", releaseId: 187, label: "Financial Stress Index",
  ...
  {
    id: "fed_balance_sheet", seriesId: "WALCL", cadence: "weekly", releaseId: 20, label: "Federal Reserve Assets",
  ...
  {
    id: "m2_money_stock", seriesId: "M2SL", cadence: "monthly", releaseId: 21, label: "M2 Money Stock",
  ...
  {
    id: "sofr", seriesId: "SOFR", cadence: "daily", releaseId: 445, label: "Secured Overnight Financing Rate",
  ...
  {
    id: "credit_spread_baa", seriesId: "BAA10Y", cadence: "daily", releaseId: 304, label: "Baa Credit Spread",
  ...
  {
    id: "credit_conditions", seriesId: "NFCICREDIT", cadence: "weekly", releaseId: 221, label: "Credit Conditions",
  ...
  {
    id: "real_yield_10y", seriesId: "DFII10", cadence: "daily", releaseId: 18, label: "10Y Real Yield",
  ...
  {
    id: "trade_weighted_dollar", seriesId: "DTWEXBGS", cadence: "daily", releaseId: 17, label: "Trade-Weighted US Dollar",
  ...
  {
    id: "housing_starts", seriesId: "HOUST", cadence: "monthly", releaseId: 27, label: "Housing Starts",
  ...
  {
    id: "building_permits", seriesId: "PERMIT", cadence: "monthly", releaseId: 27, label: "Building Permits",
  ...
  {
    id: "mortgage_rate_30y", seriesId: "MORTGAGE30US", cadence: "weekly", releaseId: 190, label: "30Y Mortgage Rate",
...
async function fetchMacroIndicator(definition: FredMacroDefinition, apiKey: string): Promise<MarketMacroIndicator> {
  // Metadata is best-effort. It was previously fetched with Promise.all, so a
  // single flaky metadata response took the whole card down - and now that it
  // is cached for a day, that would have been a long outage for a decorative
  // line of text. The card renders from the numbers alone.
  const [observations, series] = await Promise.all([
    ...
    }), CADENCE_REFRESH_SECONDS[definition.cadence]),
    fetchFredJson<FredSeriesPayload>(
      fredUrl("series", apiKey, { series_id: definition.seriesId }),
  ...
  return buildMacroIndicator(definition, parseFredObservations(observations), {
    // The pinned cadence is the fallback, so a missing metadata response costs
    // the ingest time and nothing else.
    frequency: metadata?.frequency ?? FALLBACK_FREQUENCY_LABEL[definition.cadence],
