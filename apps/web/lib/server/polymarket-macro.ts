...
export const MACRO_EVENT_MAPPINGS: readonly MacroMappingDefinition[] = [
  ...
  {
    key: "fed_year_end_rate",
    titlePattern: /^What will the Fed rate be at the end of \d{4}\?$/i,
    theme: "fed_policy",
    matchKind: "related_signal",
    matchNote: "Forward year-end target-range distribution, compared with the latest effective rate.",
    indicatorIds: ["effective_fed_funds"],
    selection: "highest_volume",
  },
  {
    key: "fed_annual_cuts",
    titlePattern: /^How many Fed rate cuts in \d{4}\?$/i,
    theme: "fed_policy",
    matchKind: "related_signal",
    matchNote: "Counts policy decisions over the calendar year rather than forecasting the effective rate directly.",
    indicatorIds: ["effective_fed_funds"],
    selection: "highest_volume",
  },
  {
    key: "gdp_full_year",
    titlePattern: /^GDP growth in \d{4}$/i,
    theme: "growth",
    matchKind: "related_signal",
    matchNote: "Contract uses full-year real GDP growth; the FRED headline card is quarterly growth at an annualized rate.",
    indicatorIds: ["real_gdp_growth"],
    selection: "highest_volume",
  },
  {
    key: "gdp_negative",
    titlePattern: /^Negative GDP growth in \d{4}\?$/i,
    theme: "growth",
    matchKind: "related_signal",
    matchNote: "Forward downside threshold related to, but not identical with, the latest quarterly GDP reading.",
    indicatorIds: ["real_gdp_growth"],
    selection: "highest_volume",
  },
  {
  ...
  {
    key: "inflation_annual_tail",
    titlePattern: /^How high will inflation get in \d{4}\?$/i,
    theme: "inflation",
    matchKind: "related_signal",
    matchNote: "Annual threshold risk complements the latest inflation readings; resolved thresholds remain visible as context.",
    indicatorIds: ["cpi_inflation", "core_pce_inflation"],
    selection: "highest_volume",
  },
  {
    key: "unemployment_annual_high",
    titlePattern: /^How high will US unemployment go in \d{4}\?$/i,
    theme: "labor",
    matchKind: "exact_series",
    matchNote: "Uses the same BLS U-3 unemployment series, but asks whether a threshold is reached at any point this year.",
    indicatorIds: ["unemployment_rate"],
    selection: "highest_volume",
  },
  {
    key: "recession_year",
    titlePattern: /^US recession by end of \d{4}\?$/i,
    theme: "recession",
    matchKind: "related_signal",
    matchNote: "Resolves from NBER or consecutive negative GDP rules; it is not the Sahm Rule itself.",
    indicatorIds: ["sahm_rule", "real_gdp_growth"],
    selection: "highest_volume",
  },
  {
    key: "mortgage_annual_range",
    titlePattern: /^Will the 30-year Mortgage Rate hit .* in \d{4}\?$/i,
    theme: "housing",
    matchKind: "exact_series",
    matchNote: "Uses the same Freddie Mac 30-year fixed-rate mortgage series as the FRED card.",
    indicatorIds: ["mortgage_rate_30y"],
    selection: "highest_volume",
  },
] as const;
