...
export async function readBackpack(assetId?:number):Promise<MonitorData>{
 const empty:MonitorData={status:'not_configured',asOf:null,assets:[],history:[],ecosystem:[],bp:[],quality:[],quotes:[],holders:[],dex:[],runs:[],usage:[]};
 ...
 const [assets,history,ecosystem,bp,quality,quotes,holders,dex,runs,usage]=await Promise.all([
  ...
  sql`SELECT *,date::text AS date FROM backpack_asset_daily_snapshots
       WHERE (${assetId??null}::bigint IS NULL OR asset_id=${assetId??null}) ORDER BY date DESC LIMIT 20000`,
  sql`SELECT *,date::text AS date FROM backpack_ecosystem_daily_snapshots ORDER BY date DESC LIMIT 5000`,
  sql`SELECT b.*, s.* ,s.date::text AS date FROM backpack_bp_daily_snapshots b
       JOIN backpack_asset_daily_snapshots s USING(asset_id,date) ORDER BY date DESC LIMIT 5000`,
  ...
  assetId?sql`SELECT h.*, l.entity,l.confidence,l.source AS label_source FROM backpack_asset_holder_daily_snapshots h
       LEFT JOIN backpack_wallet_labels l USING(wallet_address) WHERE h.date=${day}::date AND h.asset_id=${assetId}
 ...
 return {status:day?'ready':'awaiting_capture',asOf:day,assets:assets as Row[],history:(history as Row[]).reverse(),
  ecosystem:(ecosystem as Row[]).reverse(),bp:(bp as Row[]).reverse(),quality:quality as Quality[],quotes:quotes as Row[],holders:holders as Row[],dex:dex as Row[],runs:runs as Row[],usage:usage as Row[]};
}
