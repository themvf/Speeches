...
export async function readBackpack(assetId?:number):Promise<MonitorData>{
 ...
 const [assets,history,ecosystem,bp,quality,quotes,holders,dex,runs,usage,whales]=await Promise.all([
  ...
  sql`SELECT *,date::text AS date FROM backpack_bp_whale_daily_snapshots
       WHERE (${assetId??null}::bigint IS NULL OR asset_id=${assetId??null}) ORDER BY date DESC,threshold_usd LIMIT 15000`,
 ]);
