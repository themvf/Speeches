...
export function GrowthOverview({data}:{data:MonitorData}){
 ...
 <article><h3>Token issuance</h3><strong>{pct(row?.median_supply_growth_pct)}</strong><p>Median per-security supply change over {period} days. No stock prices or addition of unlike tokens.</p></article>
 <article><h3>Growth breadth</h3><strong>{pct(row?.growing_holder_breadth_pct)}</strong><p>Securities gaining holders. Expanding supply: {pct(row?.expanding_supply_pct)}. Contracting supply: {pct(row?.contracting_supply_pct)}.</p></article>
 ...
 <p>Growing slowly requires both rates above those bands, with at least {pct(settings.breadth_threshold??60)} of securities gaining holders and expanding supply. Growing rapidly also requires holder growth of at least {pct(settings.rapid_threshold??10)}. These are configurable research settings, not empirically validated universal cutoffs.</p>
 ...
 <p>Momentum requires holder and supply rates to move together by more than {String(settings.momentum_threshold??0.25)} percentage points versus the previous window. Gaps, changed assets or changed system exclusions suppress comparisons.</p>
