...
const SORT:Record<string,(a:Row,b:Row)=>number>={
 early:(a,b)=>b.early_coins-a.early_coins||Number(sampled(b))-Number(sampled(a))||(b.hit_rate??0)-(a.hit_rate??0)||(b.median_excess_24h??-Infinity)-(a.median_excess_24h??-Infinity)||b.episodes-a.episodes,
 hit:(a,b)=>Number(sampled(b))-Number(sampled(a))||(b.hit_rate??-1)-(a.hit_rate??-1)||b.episodes-a.episodes,
 move:(a,b)=>Number(sampled(b))-Number(sampled(a))||(b.median_excess_24h??-Infinity)-(a.median_excess_24h??-Infinity)||b.episodes-a.episodes,
 evidence:(a,b)=>b.episodes-a.episodes||b.coins.length-a.coins.length,
 audience:(a,b)=>(b.followers??-1)-(a.followers??-1),
 recent:(a,b)=>String(b.last_at??'').localeCompare(String(a.last_at??'')),
};
export function CryptoPeopleView({initialCoin,onAccount}:{initialCoin:string|null;onAccount:(id:string)=>void}){
 ...
 const [coin,setCoin]=useState(initialCoin??'ALL'),[rank,setRank]=useState('early'),[style,setStyle]=useState('any'),[minEp,setMinEp]=useState(0),[audience,setAudience]=useState(0),[active,setActive]=useState(0),[search,setSearch]=useState('');
 ...
 const shown=useMemo(()=>{const since=active?Date.now()-active*86400000:0;
  return (rows??[]).filter(r=>(coin==='ALL'||r.coins.includes(coin))&&r.episodes>=minEp&&(r.followers??0)>=audience&&(!since||(r.last_at&&Date.parse(r.last_at)>=since))&&r.handle.toLowerCase().includes(search.toLowerCase())&&(STYLE_FILTER[style]??STYLE_FILTER.any)(r))
   .sort(SORT[rank]??SORT.early);},[rows,coin,rank,style,minEp,audience,active,search]);
 return <div style={{display:'flex',flexDirection:'column',gap:18}}>
  ...
  <section className={styles.wsSection}><div className={styles.table}><table className={styles.peopleTable}><thead><tr><th>Who</th><th>Where early</th><th>After they post</th><th>How they post</th><th>Evidence</th></tr></thead><tbody>{shown.slice(0,100).map((r,i)=>{const early=earlyCoins(r),after=afterPosting(r),ways=postingStyles(r);return <tr key={r.account_id} className={sampled(r)?'':styles.dimRow}>
   ...
   <td><span style={{whiteSpace:'nowrap'}}>{r.episodes} price-linked · {r.coins.length} coin{r.coins.length===1?'':'s'} · {r.days} day{r.days===1?'':'s'}</span><small className={styles.cellNote}>{whyLeader(r)}</small></td>
  </tr>;})}</tbody></table></div>
