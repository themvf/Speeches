...
import {computeRings,dayOneCircle,circleEvidence,RING_LABEL,RING_DESCRIPTION,type Ring,type CircleEntry} from '@/lib/crypto-rings';
...
// Day-1 supporter badge: the account's first post on a coin landed the day of that coin's first saved contract post.
const D1=({l}:{l:Leader|null|undefined})=>{const c=l?dayOneCoins(l):[];return c.length?<span className={s.d1} title={'Day-1 supporter of '+c.join(', ')}>d1{c.length>1?'×'+c.length:''}</span>:null;};
const TREND_TONE:Record<string,string>={rising:s.pos,fading:s.neg,gone:s.neg,steady:'',new:s.faint};
// Day-1 circle badge: replied to, quoted or tagged by a day-1 supporter of the focus coin. Peers post on the coin; outside targets do not.
const Circle=({c}:{c:CircleEntry|undefined})=>c?<span className={`${s.d1} ${c.inside?s.d1Peer:s.d1Out}`} title={'Day-1 circle: '+circleEvidence(c)}>{c.inside?'d1·peer':'d1·target'}</span>:null;
...
export function CryptoWorkbench(){
 ...
 // Day-1 supporters of the focus coin, then everyone they talk to in the scoped posts.
 const dayOneIds=useMemo(()=>new Set((leaders??[]).filter(l=>l.roles.some(r=>r.coin===focusCoin&&r.early&&r.day===1)).map(l=>l.account_id)),[leaders,focusCoin]);
 const circle=useMemo(()=>dayOneCircle(scoped,dayOneIds),[scoped,dayOneIds]);
 const circleCounts=useMemo(()=>{let peer=0,out=0;for(const c of circle.values())c.inside?peer++:out++;return {peer,out};},[circle]);
 ...
 // People
 const people=useMemo(()=>(leaders??[]).filter(l=>matchesQuery(l,query,coin,rings,circle)),[leaders,query,coin,rings,circle]);
 ...
 const feed=useMemo(()=>scoped.filter(p=>p.kind!=='repost'&&(!query.handle||p.handle.toLowerCase().includes(query.handle))&&(query.ring==null||rings.get(p.author_id)?.ring===query.ring)).sort((a,b)=>b.posted_at.localeCompare(a.posted_at)).slice(0,400),[scoped,query.handle,query.ring,rings]);
 const day1Count=useMemo(()=>(leaders??[]).filter(l=>(!state.coin||l.coins.includes(state.coin))&&dayOneCoins(l).length).length,[leaders,state.coin]);
 const multiCount=useMemo(()=>(leaders??[]).filter(l=>(!state.coin||l.coins.includes(state.coin))&&l.coins.length>2).length,[leaders,state.coin]);
 const leaderById=useMemo(()=>new Map((leaders??[]).map(l=>[l.account_id,l])),[leaders]);
 ...
 return <div className={s.wb}>
  ...
  <div className={s.body}>
   ...
   <div className={s.col} ref={centre}>
    ...
    {(state.tab==='people'||state.tab==='posts')&&<div className={s.ringRow}><span className={s.faint}>Rings · {focusCoin} · {fromDay}→{toDay}</span>{RINGS.filter(r=>ringCounts.get(r)).map(r=><button key={r} className={s.ringTag} data-ring={r} title={RING_DESCRIPTION[r]} aria-pressed={query.ring===r} onClick={()=>setQText(q=>(q.replace(/\bring:?\d\b/gi,'').trim()+(query.ring===r?'':' ring:'+r)).trim())}>{r} {RING_LABEL[r]} · {ringCounts.get(r)}</button>)}
     <button className={`${s.ringTag} ${s.d1Chip}`} title="Accounts whose first post on a coin landed the day of its first saved contract post" aria-pressed={query.day1} onClick={()=>setQText(q=>(q.replace(/\b(day1|d1)\b/gi,'').trim()+(query.day1?'':' day1')).trim())}>d1 day-1 · {day1Count}</button>
    ...
    {state.tab==='people'&&<div className={s.scroll}>{!leaders?<div className={s.empty}>Reading saved rankings…</div>:!rows.length?<div className={s.empty}>No accounts match. Rankings appear after the next collection run; price-linked posts need a day of hourly candles.</div>:
     ...
     <tbody>{rows.slice(0,300).map((row,i)=>{const r=row.original,early=earlyCoins(r),ok=sampled(r),styles=postingStyles(r);return <tr key={r.account_id} className={`${s.row} ${ok?'':s.dim} ${state.account===r.account_id?s.on:''} ${i===cursor?s.cursor:''}`} onClick={()=>{setCursor(i);set({account:r.account_id});}} title={whyLeader(r)}>
      <td className={s.faint}>{i+1}</td><td><D1 l={r}/><Circle c={circle.get(r.account_id)}/><span className={s.link}>@{r.handle}</span></td><td className={`${s.r} ${s.soft}`}>{aud(r.followers)}</td>
    ...
    {state.tab==='posts'&&<><div className={`${s.pad} ${s.faint}`} style={{fontSize:11,borderBottom:'1px solid var(--line-soft)',display:'flex',flexWrap:'wrap',gap:'2px 10px'}}><span>X posts on {focusCoin} · {fromDay} → {toDay} · newest first · reposts hidden{query.handle?` · @${query.handle}`:''}{query.ring!=null?` · ring ${query.ring} ${RING_LABEL[query.ring]} only`:''}</span><span style={{marginLeft:'auto'}}>{run?`${feed.length}${feed.length===400?'+':''} posts · tap one to read it in full, X ↗ to open it on X · next close vs post day`:''}</span></div>
     <div className={s.scroll}>{!run?<div className={s.empty}>Loading saved posts…</div>:!feed.length?<div className={s.empty}>No saved posts in scope.</div>:feed.map(p=>{const open=openPost===p.id;return <div key={p.id} id={'wbp-'+p.id} className={`${s.feedRow} ${s.postRow} ${open?s.postOpen:''}`} role="button" tabIndex={0} aria-expanded={open} onClick={()=>setOpenPost(open?null:p.id)} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();setOpenPost(open?null:p.id);}}} title={open?undefined:p.text}>
      ...
      <a className={s.openX} href={p.url} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} title={`Open @${p.handle}'s post on X`} aria-label={`Open @${p.handle}'s post on X`}>X ↗</a>
      {open&&<span className={s.postActions} onClick={e=>e.stopPropagation()}><a href={p.url} target="_blank" rel="noreferrer" style={{fontSize:11,fontWeight:600}}>Open this post on X ↗</a><button className={s.link} style={{fontSize:11}} onClick={()=>set({account:p.author_id})}>Account @{p.handle} →</button><span className={s.faint} style={{fontSize:11}}>{p.kind==='quote'?'quote':p.kind==='reply'?'reply':'post'} · next close {nextClose(p.posted_at)==null?'—':pct(nextClose(p.posted_at),0)}</span></span>}</div>;})}</div></>}
   ...
   <div className={`${s.col} ${s.right}`}>
    ...
    :!acct?<div className={s.empty}>Loading account…</div>:acct.status!=='ready'?<div className={s.empty}>No saved evidence for this account. <button className={s.link} onClick={()=>set({account:null})}>close</button></div>:<>
     <div className={s.paneHead}><D1 l={summary}/><Circle c={acct.account?circle.get(acct.account.id):undefined}/><strong>@{acct.account?.handle}</strong><span className={s.faint} style={{fontSize:11}}>{aud(acct.latest_profile?.followers??acct.account?.followers)} followers</span><a href={`https://x.com/i/user/${acct.account?.id}`} target="_blank" rel="noreferrer" style={{marginLeft:'auto',fontSize:11}}>x.com ↗</a><button className={s.link} style={{fontSize:11,fontWeight:500}} onClick={()=>set({account:null})}>close</button></div>
     <div className={s.paneWhy}>{(()=>{const rg=acct.account?rings.get(acct.account.id):undefined;return rg?.ring?<div style={{marginBottom:4}}><span className={s.ringTag} data-ring={rg.ring}>ring {rg.ring} · {RING_LABEL[rg.ring]}</span> <span className={s.faint}>on {focusCoin} {fromDay}→{toDay}: {rg.evidence}{rg.tags.length>1?' · also '+rg.tags.filter(t=>t!==rg.ring).map(t=>RING_LABEL[t]).join(', '):''}</span></div>:null;})()}{summary?whyLeader(summary):'saved posts only'}{(()=>{const id=acct.account?.id;if(!id)return null;const c=circle.get(id);if(c)return <div className={s.faint} style={{marginTop:4}}>Day-1 circle on {focusCoin}: {circleEvidence(c)}.</div>;
      if(!dayOneIds.has(id))return null;const talks=[...circle].filter(([,x])=>x.from.has(acct.account!.handle)).sort((a,b)=>(b[1].from.get(acct.account!.handle)??0)-(a[1].from.get(acct.account!.handle)??0)).slice(0,12);
      return talks.length?<div className={s.faint} style={{marginTop:4}}>Talks to on {focusCoin}: {talks.map(([tid,x])=><span key={tid}><button className={s.link} style={{fontSize:11}} onClick={()=>set({account:tid})}>@{x.handle}</button><span className={x.inside?s.pos:s.warn}>{x.inside?' peer':' target'}</span> ×{x.from.get(acct.account!.handle)}{' · '}</span>)}</div>:null;})()}{acct.latest_profile?.bio?<span className={s.faint} style={{display:'block',marginTop:4,whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{acct.latest_profile.bio}</span>:null}</div>
     <div className={s.h}>Per coin</div>
     <div className={s.fixed}><table className={s.t}><thead><tr><th>Coin</th><th>Early</th><th className={s.r}>Posts</th><th className={s.r}>Up</th><th className={s.r}>Excess</th></tr></thead><tbody>{(acct.activity??[]).map(a=>{const role=summary?.roles.find(r=>r.coin===a.coin),imp=summary?.impact.find(i=>i.coin===a.coin);const ok=(imp?.episodes??0)>=MIN_LINKED_POSTS;return <tr key={a.coin} className={s.row} onClick={()=>set({coin:a.coin,tab:'timeline',highlight:acct.account?.id??null})}><td style={{fontWeight:600,color:'#7dd3fc'}}>{a.coin}</td><td className={`${s.mono} ${s.soft}`}>{role?.early&&role.day?'d'+role.day+(role.contract?' ·ca':''):'—'}</td><td className={s.r}>{a.posts}</td><td className={s.r}>{imp?`${Math.round((imp.share_up_24h??0)*imp.episodes)}/${imp.episodes}`:'—'}</td><td className={`${s.r} ${ok?tone(imp?.median_excess_24h):s.faint}`}>{ok?pct(imp?.median_excess_24h,0):'—'}</td></tr>;})}</tbody></table>{summary&&afterPosting(summary)==null&&summary.episodes>0&&<div className={`${s.pad} ${s.faint}`} style={{fontSize:11}}>Too few price-linked posts to read the outcome.</div>}
     {(acct.activity??[]).length>1&&<div className={`${s.pad} ${s.soft}`} style={{fontSize:11}}>On {(acct.activity??[]).length} coins{summary&&dayOneCoins(summary).length?` · day-1 on ${dayOneCoins(summary).join(', ')}`:''}</div>}
     <div className={s.h}>Interest over time <span>· posts per week since first post</span></div>
     {(acct.activity??[]).map(a=>{const t=interestTrend((acct.weekly??[]).filter(w=>w.coin===a.coin),a.last_at);const mx=Math.max(1,...t.bars);return <div key={a.coin} className={s.cadence} title={`${a.coin}: first two weeks ${t.opening} posts, latest two weeks ${t.recent}`}>
      <span style={{fontWeight:600,color:'#7dd3fc',width:74}}>{a.coin}</span>
      <span className={s.bars} aria-hidden>{t.bars.slice(-16).map((b,i)=><i key={i} style={{height:Math.max(2,Math.round(b/mx*14))}}/>)}</span>
      <span className={TREND_TONE[t.label]} style={{fontWeight:600,width:46}}>{t.label}</span><span className={s.faint}>{t.lastDays==null?'':t.lastDays===0?'today':t.lastDays+'d ago'}</span></div>;})}</div>
