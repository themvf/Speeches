...
export function CryptoSocialPanel() {
 ...
 return <section className={panel} aria-label="Crypto social research">
  ...
  <p className="text-xs text-[color:var(--ink-faint)]">Saved research only. Opening this panel consumes no X API credits. Seven-day sample; counts are observed matches, not total X volume.</p>
  ...
  {error?<p role="alert">{error}</p>:!data?<p>Loading saved research…</p>:data.status!=="ready"?<p>The pilot has not collected data yet. Run the manual Crypto Social Research Pilot workflow after configuring its credentials.</p>:<>
   ...
   <div className="overflow-x-auto"><table className="w-full text-left text-xs"><caption className="text-left py-2 font-semibold">Candidate influencers · sampled coin activity</caption>
