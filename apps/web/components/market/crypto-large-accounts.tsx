...
export function CryptoLargeAccounts({posts,suggestedDay,onSelect}:{posts:RunPost[];suggestedDay:string;onSelect:(id:string)=>void}){
 ...
 return <div>
  ...
  {shown.length?<>
   ...
   <div className={styles.reachMobile}>{shown.map(a=><article key={a.id} className={styles.mobileAccount}><button className={styles.accountRowButton} onClick={()=>onSelect(a.id)}><div className={styles.mobileAccountHeader}><strong>@{a.handle}</strong><span><b className={styles.followerValue}>{compact(a.followers)}</b> followers</span></div><span>{a.posts} coin posts · First saved {a.first.posted_at.slice(0,10)}</span><small>View posts & connections →</small></button></article>)}</div>
