...
import { capitalFormationKeywordRegex } from "@/lib/capital-formation-vocabulary";
import {
  decodeEntities,
  getMatchingTopics,
  normalizeMatchText,
...
// Derived from capital-formation-vocabulary.json rather than hand-maintained,
// so this fallback can no longer drift from the topic rule it stands in for.
// "listing" and "share sale" are kept as extra loose terms because this path
// only runs for documents that produced no real topic match at all.
const CAPITAL_FORMATION_FALLBACK_RE = capitalFormationKeywordRegex();
const CAPITAL_FORMATION_FALLBACK_LOOSE_RE = /\b(share sale|listing)\b/;
...
function fallbackDocumentTopicMatches(article: FeedItem, rules: TopicRuleView[]): TopicRuleView[] {
  ...
  if (CAPITAL_FORMATION_FALLBACK_RE.test(text) || CAPITAL_FORMATION_FALLBACK_LOOSE_RE.test(text)) {
...
export function IntelBetaDashboard({
  ...
  const feedItems = useMemo<FeedItem[]>(
    () => {
      ...
      return [...articles, ...documentFeedItems]
        .filter((item) => {
