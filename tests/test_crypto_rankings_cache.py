...
def test_db_snapshots_cover_every_coin_and_are_replaced_in_place(db):
    ...
    assert 'voices:ZCAT' in keys and 'watchers:ALL' in keys and 'watchers:ZEC' in keys and len(keys)==11
    assert next(w for w in written if w['key']=='voices:ZCAT')['rows']==1
    # Watcher rows are loaded once and reused for every coin scope.
    assert len([c for c in calls if c['kind']=='watchers'])==6 and all(len(c['posts'])==1 for c in calls if c['kind']=='watchers')
    with db,db.cursor() as c:
